...
useEffect(() => {
    ...
    <a href="#" className="text-3xl font-semibold">
        Rimma A Website-Portfolio
        </a>

    {/* Кросс верстка для мобильной версии */}
        ...
        <ul className="text-center space-y-6 p-7">
            {navItems.map(item => (
                <li key={item.id}>
                    ...
                    onClick={()=> handleLonkClick(item.id)}
                    >
                        {item.label}</a>
                </li>
            ))}
    ...
    {/* Меню */}
        ...
        {/* Пункты меню*/}
        <button className="lg:hidden relative h-7 w-8 flex items-center jusify-center" onClick={() => setIsOpen(!isOpen)}>
